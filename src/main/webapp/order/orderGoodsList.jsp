...
                },
        ...
        $('#data-tables').bootstrapTable({
	    	url: '<%=basePath%>projectOrder/mismatch.action',
			method: 'post',
			contentType: "application/x-www-form-urlencoded",
			/* queryParamsType的默认值为 'limit' ,在默认情况下 传给服务端的参数为：offset,limit,sort  设置为 ''  在这种情况下传给服务器的参数为：pageSize,pageNumber */
            queryParamsType:'', 
            ...
            sidePagination: "server", //分页方式：client客户端分页，server服务端分页（*）
            showRefresh:false,//刷新按钮
            height:tableHeight-80,//table的高度
            //按需求设置不同的样式：5个取值代表5中颜色['active', 'success', 'info', 'warning', 'danger'];
			rowStyle:'success',
			/* function (row, index) {
			    var style = "";
...
                },
	...
	function pass() {
		$.post(
			"<%=basePath%>projectOrder/selectStatus?projectOrderId="+<%=projectOrderId%>,
			function(data){
				if(data==0){
	...
	function reject() {
		...
		if(rejectReason==""||rejectReason==null){
			...
			$.post(
				"<%=basePath%>sysFlow/reject.action?sysFlowId=<%=sysFlowId%>&rejectReason="+$('#rejectReason').val(),
				function(data){
					if(data==1){
						alert("退回成功！");
						toProjectList();
