...
function getAuth() {
	$("#sysRole").val(<%=sysRoleId%>);
	$.ajax({
		url : '<%=basePath%>sysRole/getRangeViewTree.action',
		dataType : 'json',
		type : 'post',
		...
		success:function(data){
			...
			$.ajax({
				url : '<%=basePath%>sysResource/sysResourceSelected.action',
				dataType : 'json',
				type : 'post',
				...
				success:function(data){
					if(data.length > 0){
						for(var i=0;i<data.length;i++){
		 					var obj= $('#tree').treeview('getNode', data[i]);
		 					$(obj).attr("showCheckbox",false);
							obj.color="red";
						}
						$('#tree').treeview('expandAll', { levels: 9, silent: true });
					}
				},
				error:function(){
					alert("请求失败！");
...
function saveAuth(){
		...
		$.ajax({
			url:'<%=basePath%>sysRole/saveRoleRange.action',
			dataType:'json',
			type:'post',
			traditional:true,
			data:{
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title" id="myModalLabel">分配角色一次性权限</h4>
				</div>
				<div class="container">
					<form class="form-horizontal" id="authForm" method="post">
						<div class="form-group">
							<label class="col-md-2 control-label">分配角色一次性权限：</label>
							<div class="col-md-3 ">
								<input type="hidden" id="sysRole" name="sysRole">
								<div id="tree" style="height:450px;width:300px;overflow: auto;"></div>
							</div>
						</div>
						<div class="modal-footer col-md-6">
							<span><font color="red" >红色字体为已分配角色权限，不用配置一次性权限范围&nbsp;&nbsp;&nbsp;</font></span>
							<!--用来清空表单数据-->
